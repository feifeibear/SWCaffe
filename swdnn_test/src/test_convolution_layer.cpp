...
void caffe_conv(const Blob<Dtype>* in, ConvolutionParameter* conv_param,
  ...
  DLOG(INFO) << "o_g " << o_g << " dilation_d " << dilation_d
    << " dilation_h " << dilation_h << " dilation_w " << dilation_w
    << " groups " << groups << " has_depth " << has_depth; 

  DLOG(INFO) << "o_g " << o_g << " k_g " << k_g 
    << " out->shape(0) " << out->shape(0) << " out->shape(1) " << out->shape(1)
    << " out->shape(2) " << out->shape(2) << " out->shape(3) " << out->shape(3);
  DLOG(INFO) << " in->shape(0) " << in->shape(0) << " in->shape(1) " << in->shape(1)
    << " in->shape(2) " << in->shape(2) << " in->shape(3) " << in->shape(3);

   DLOG(INFO) << " weights[0]->shape(0) " << weights[0]->shape(0) << " weights[0]->shape(1) " << weights[0]->shape(1)
    << " weights[0]->shape(2) " << weights[0]->shape(2) << "weights[0]->shape(3) " << weights[0]->shape(3); 

   DLOG(INFO) << "kernel_d " << kernel_d << " kernel_h " << kernel_h << " kernel_w " << kernel_w;
...
TYPED_TEST(ConvolutionLayerTest, TestSimpleConvolution) {
  ...
  //FJR subtitude with my conv impl
...
TYPED_TEST(ConvolutionLayerTest, TestSWBackward) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  Blob<Dtype> backward_bias_result_2d;
  // Test with 2D im2col
  {
