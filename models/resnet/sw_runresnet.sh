...
mkdir -p ./log && bsub -b -I -J sw_resnet_debug_zwl -q q_sw_share -host_stack 1024  -N $1 -np 1 -cgsp 64 -sw3run ./sw3run-all -sw3runarg "-a 1" -cross_size 28000 ./bin/caffe_sw train --solver=models/resnet/solver_debug.prototxt 2>&1 | tee ./log/sw_resnet_`date '+%y-%m-%d-%H-%M-%S'`.log
