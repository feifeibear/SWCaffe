...
int main (int argc, char ** argv) {
  ...
  DummyDataParameter* dummyparam = datalayerparam.mutable_dummy_data_param();
  BlobShape* datashape = dummyparam->add_shape();
  datashape->add_dim(5);
  datashape->add_dim(2);
  datashape->add_dim(3);
  datashape->add_dim(4);
  FillerParameter* filler = dummyparam->add_data_filler();
  filler->set_type("gaussian");
  filler->set_std(0.01);
  std::cout << "dummy data is OK" << std::endl;
