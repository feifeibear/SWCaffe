...
void RecurrentLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO)<<"total layer number = "<<num_layer;
  for (int i=0; i<num_layer; i++){
    LOG(INFO)<<net_param.layer(i).name();
    int num_top = net_param.layer(i).top_size();
    LOG(INFO)<<"total top number = "<<num_top;
    for (int j=0; j<num_top; j++)
      LOG(INFO)<<net_param.layer(i).top(j);
    int num_bottom = net_param.layer(i).bottom_size();
    LOG(INFO)<<"total bottom number = "<<num_bottom;
    for (int j=0; j<num_bottom; j++)
      LOG(INFO)<<net_param.layer(i).bottom(j);
  }*/
