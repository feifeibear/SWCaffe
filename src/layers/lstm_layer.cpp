...
void LSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  //LOG(INFO)<<"cont_slice_param "<<cont_slice_param;
  //LOG(INFO)<<"haha "<<(&(net_param->layer(2)));
  //LOG(INFO)<<"cont slice index = "<<net_param->layer_size()-1;
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    //LOG(INFO)<<"layer "<<net_param->layer(2).name()<<" has "<<net_param->layer(2).top_size()<<" top!";
  ...
  //LOG(INFO)<<"cont_slice top number = "<<cont_slice_param->top_size();
  //LOG(INFO)<<"layer "<<net_param->layer(2).name()<<" has "<<net_param->layer(2).top_size()<<" top!";
}
