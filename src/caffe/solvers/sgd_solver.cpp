...
//#endif
  ...
  case Caffe::CPU: {
    ...
    caffe_copy(net_params[param_id]->count(),
       ...
       net_params[param_id]->mutable_cpu_diff());
    //printf("rank %d param_id :%d\n",Caffe::mpi_rank(),param_id);
      //for(int i = 0; i < net_params[param_id]->count(); ++i) {
      //if((param_id>= 30)&& (Caffe::mpi_rank()%2 ==0))
        //printf( "rank %d history_[%d] : %f \n",Caffe::mpi_rank(),param_id, history_[param_id]->mutable_cpu_data()[i] );
      //history_[param_id]->mutable_cpu_data()[i] = 1.0; 
      //float tmp = 
        //net_params[param_id]->cpu_diff()[i] * local_rate + momentum; //[>history_[param_id]->mutable_cpu_data()[i];
      //net_params[param_id]->mutable_cpu_diff()[i] = 1.*local_rate*momentum; //history_[param_id]->cpu_data()[i];
    //}
    break;
  }
  case Caffe::GPU: {
