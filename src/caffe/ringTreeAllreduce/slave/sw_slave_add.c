...
void sw_slave_add_f_allreduce(addPara *para) {
...
void sw_slave_add_f_small(addPara *para) {
  int id = athread_get_id(-1);
  int spenum = 48;
  if(id > spenum)
    return;
  ...
  SIMDTYPEF vdst;
  int count = para->count;
  int local_count = count/spenum + (id<(count%spenum));
  int start = id*(count/spenum) + (id<(count%spenum)?id:(count%spenum));
  float * src_ptr1 = &(((float *)para->src1)[start]);
  float * src_ptr2 = &(((float *)para->src2)[start]);
  float * dst_ptr  = &(((float *)para->dst)[start]);
  volatile int replyget=0, replyput=0;
