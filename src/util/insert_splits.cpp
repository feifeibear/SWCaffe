...
void InsertSplits(const NetParameter& param, NetParameter* param_split) {
  ...
  for (int i = 0; i < param.layer_size(); ++i) {
    //LOG(INFO)<<param.layer(i).name();
    const LayerParameter& layer_param = param.layer(i);
    layer_idx_to_layer_name[i] = layer_param.name();
    for (int j = 0; j < layer_param.top_size(); ++j) {
      const string& blob_name = layer_param.top(j);
      //LOG(INFO)<<"add top blob "<<blob_name<<" in layer "<<layer_param.name();
      blob_name_to_last_top_idx[blob_name] = make_pair(i, j);
    }
  }
  //LOG(FATAL)<<"BREAKPOINT";
  for (int i = 0; i < param.layer_size(); ++i) {
    const LayerParameter& layer_param = param.layer(i);
    layer_idx_to_layer_name[i] = layer_param.name();
    for (int j = 0; j < layer_param.bottom_size(); ++j) {
      const string& blob_name = layer_param.bottom(j);
      //LOG(INFO) << "Bottom blob '" << blob_name << "' (layer '"
      //             << layer_param.name() << "', bottom index " << j << ")";
      //LOG(INFO)<<"size="<<blob_name_to_last_top_idx.size();
      if (blob_name_to_last_top_idx.find(blob_name) ==
          blob_name_to_last_top_idx.end()) {
        //for (map<string,pair<int,int> >::iterator iter=blob_name_to_last_top_idx.begin(); iter!=blob_name_to_last_top_idx.end(); iter++)
        //  LOG(INFO)<<"blob "<<(*iter).first;
