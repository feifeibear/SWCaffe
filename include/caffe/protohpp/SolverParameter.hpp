...
public:
    ...
    snapshot_format_ = SolverParameter_SnapshotFormat_BINARYPROTO;
  ...
  void CopyFrom(const SolverParameter& other) {
    ...
    if (other.test_net_param().size() > 0) {
      test_net_param_.resize(num_test_);
      for (int i=0; i<num_test_; i++) 
  ...
  inline const std::vector<NetParameter>& test_net_param() const { return test_net_param_; }
  ...
  inline std::string regularization_type() const { regularization_type_; }
  inline int stepsize() const { return stepsize_; }
  inline int stepvalue() const { return stepvalue_; }
